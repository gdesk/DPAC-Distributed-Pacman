...
class PlaygroundTest extends FunSuite{
  ...
  test("blocks, eatables and streets"){
    playground.ground = List(Apple("Apple", PointImpl(0, 0)),
                             ...
                             GalaxianShip("GalaxianShipple", PointImpl(0, 4)),
    ...
    assert(playground.eatables contains GalaxianShip("GalaxianShipple", PointImpl(0, 4)))
    ...
    for(i <- 0 until playground.dimension.x) {
      for(j <- 0 until playground.dimension.y) {
        if(!playground.blocks.exists(e => e.position equals PointImpl(i, j))) assert(playground.streetPositions contains PointImpl(i,j))
      }
    }
  ...
  test ("elementAtPosition"){
    ...
    for(i <- 0 until playground.dimension.x) {
      for(j <- 0 until playground.dimension.y) {
        if(!playground.ground.exists(e => e.position equals PointImpl(i, j))) assert(playground.elementAtPosition(PointImpl(i,j)) isEmpty)
      }
    }
  ...
  test ("singleton"){
   assert(playground == PlaygroundImpl)
  }
